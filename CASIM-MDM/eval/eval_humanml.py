    ...
    for motion_loader_name, motion_loader in motion_loaders.items():
        ...
        with torch.no_grad():
            for idx, batch in enumerate(motion_loader):
                ...
                # MODIFY make nan to 0
                # nan values will be in a row, so change the whole row to 0
                if torch.isnan(motion_embeddings).any():
                    motion_embeddings[torch.isnan(motion_embeddings)] = 0
                    print(f'---> [{motion_loader_name}] Skip NaN case')
...
def evaluate_diversity(activation_dict, file, diversity_times):
    ...
    for model_name, motion_embeddings in activation_dict.items():
        ...
        if np.isnan(motion_embeddings).any():
            motion_embeddings = motion_embeddings[~np.isnan(motion_embeddings).any(axis=1)]
            num_nan = np.isnan(motion_embeddings).any(axis=1).sum()
            print(f'---> [{model_name}] Skip {num_nan} NaN cases')
...
def collect(gt_loader, eval_motion_loaders, replication_times, diversity_times, mm_num_times, run_mm=False):
    for motion_loader_name, motion_loader_getter in eval_motion_loaders.items():
        motion_loader, mm_motion_loader = motion_loader_getter()
...
if __name__ == '__main__':
    ...
    elif args.eval_mode == 'no_sample_limit':
    ...
    elif args.eval_mode == 'collect': # collect generated samples and attention weights
        num_samples_limit = None
        run_mm = False
        mm_num_samples = 0
        mm_num_repeats = 0
    ...
    if args.eval_mode == 'collect':
        collect(gt_loader, eval_motion_loaders, replication_times, diversity_times, mm_num_times, run_mm=run_mm)
        exit()
